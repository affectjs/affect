...
describe("Backend Adapters", () => {
    describe("fluentFfmpegBackend", () => {
        it("should have correct name and supported types", () => {
            ...
            expect(fluentFfmpegBackend.supportedTypes).toEqual([
                "video",
                "audio",
            ]);
        });
        ...
        it("should handle video media type", () => {
            expect(
                fluentFfmpegBackend.canHandle({ type: "resize" }, "video")
            ).toBe(true);
        });

        it("should handle audio media type", () => {
            expect(
                fluentFfmpegBackend.canHandle({ type: "encode" }, "audio")
            ).toBe(true);
        });

        it("should not handle image media type", () => {
            expect(
                fluentFfmpegBackend.canHandle({ type: "resize" }, "image")
            ).toBe(false);
        });
        ...
        it("should execute resize operation", async () => {
            ...
            vi.doMock("@luban-ws/fluent-ffmpeg", () => ({
                default: mockFfmpeg,
            }));
    ...
    describe("sharpBackend", () => {
        ...
        it("should handle image media type", () => {
            expect(sharpBackend.canHandle({ type: "resize" }, "image")).toBe(
                true
            );
        });

        it("should not handle video media type", () => {
            expect(sharpBackend.canHandle({ type: "resize" }, "video")).toBe(
                false
            );
        });

        it("should not handle audio media type", () => {
            expect(sharpBackend.canHandle({ type: "encode" }, "audio")).toBe(
                false
            );
        });
    });
