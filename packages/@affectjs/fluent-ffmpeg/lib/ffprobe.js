...
module.exports = function (proto) {
    ...
    proto.ffprobe = function () {
        ...
        this._getFfprobePath(function (err, path) {
            ...
            var hasOutputFormat = options.some(function (opt) {
                return (
                    typeof opt === "string" &&
                    (opt === "-of" ||
                        opt === "-print_format" ||
                ...
                if (processExited && stdoutClosed && stderrClosed) {
                    ...
                    [data.format]
                        ...
                        .forEach(function (target) {
                            if (target) {
                                var legacyTagKeys =
                                    Object.keys(target).filter(legacyTag);

                                if (legacyTagKeys.length) {
                                    target.tags = target.tags || {};

                                    legacyTagKeys.forEach(function (tagKey) {
                                        target.tags[tagKey.substr(4)] =
                                            target[tagKey];
                                ...
                                if (legacyDispositionKeys.length) {
                                    ...
                                    legacyDispositionKeys.forEach(function (
                                        dispositionKey
                                    ) {
                                        target.disposition[
                                            dispositionKey.substr(12)
            ...
            ffprobe.on("exit", function (code, signal) {
                ...
                if (code) {
                    ...
                    handleExit(
                        new Error("ffprobe was killed with signal " + signal)
                    );
                } else {
                    handleExit();
                }
