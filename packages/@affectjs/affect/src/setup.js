...
function isVersionSupported(version) {
    ...
    if (version.major === MIN_VERSION_MAJOR) {
        ...
        if (
            version.minor === MIN_VERSION_MINOR &&
            version.patch >= MIN_VERSION_PATCH
        ) {
            return true;
        }
    }
...
function installFfmpeg(options, callback) {
    ...
    function doInstall(manager, installCommand) {
        if (!options.autoInstall) {
            ...
            return callback(
                new Error(
                    "ffmpeg installation required. Run: " + installCommand
                )
            );
        }

        var spinner = options.silent
            ? null
            : ora("Installing ffmpeg using " + manager + "...").start();
                                    ...
                                    if (isVersionSupported(version)) {
                                        ...
                                        if (spinner) {
                                            spinner.warn(
                                                "Installed FFmpeg " +
                                                    version.full +
                                                    " may not meet requirements (6.1.1+)"
                                            );
                                            console.warn(
...
function setup(options, callback) {
    ...
    var spinner = options.silent
        ? null
        : ora("Searching for ffmpeg...").start();
    ...
    findExecutable("ffmpeg", function (err, ffmpegPath) {
        ...
        if (err || !ffmpegPath) {
            ...
            if (options.install) {
                ...
                installFfmpeg(
                    { autoInstall: true, silent: options.silent },
                    function (installErr) {
                        if (installErr) {
                            if (spinner) {
                                spinner.fail(
                            ...
                            findExecutable("ffmpeg", function (err, newPath) {
                                if (newPath) {
                                    ...
                                    if (spinner) {
                                        spinner.fail(
                                            "FFmpeg installation completed but binary not found in PATH"
                                        );
                                    }
                                    if (options.required) {
                                        return callback(
                ...
                if (options.required) {
                    return callback(
                        new Error("ffmpeg is required but not found"),
                        config
                    );
                }
        ...
        function checkVersionAndFfprobe(ffmpegPath) {
            ...
            var versionSpinner = options.silent
                ? null
                : ora("Checking FFmpeg version...").start();
            ...
            getFfmpegVersion(ffmpegPath, function (err, version) {
                    ...
                    if (versionSpinner) {
                        if (isSupported) {
                            ...
                            versionSpinner.warn(
                                "FFmpeg " +
                                    version.full +
                                    " (below 6.1.1+, not fully supported)"
                            );
                        }
                    }
                    ...
                    if (!isSupported) {
                        if (options.install) {
                            var installSpinner = options.silent
                                ? null
                                : ora(
                                      "Attempting to install/update ffmpeg..."
                                  ).start();
                            installFfmpeg(
                                {
                                    autoInstall: true,
                ...
                function checkFfprobe() {
                    var probeSpinner = options.silent
                        ? null
                        : ora("Searching for ffprobe...").start();
                    ...
                    findExecutable("ffprobe", function (err, ffprobePath) {
                        if (err || !ffprobePath) {
                            ...
                            if (options.required) {
                                return callback(
                                    new Error(
                                        "ffprobe is required but not found"
                                    ),
                                    config
                                );
                            }
        ...
        function checkFfprobe() {
            findExecutable("ffprobe", function (err, ffprobePath) {
                if (err || !ffprobePath) {
                    ...
                    if (options.required) {
                        return callback(
                            new Error("ffprobe is required but not found"),
                            config
                        );
                    }
                } else {
