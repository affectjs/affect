...
describe("DSL Parser - Exhaustive Tests", () => {
    describe("Basic AffectBlock Syntax", () => {
        it("should parse video with hardcoded paths", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { resize 1280 720 }';
        ...
        it("should parse audio with hardcoded paths", () => {
            const dsl =
                'affect audio "input.wav" "output.mp3" { encode mp3 192 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse image with hardcoded paths", () => {
            const dsl =
                'affect image "input.jpg" "output.jpg" { resize 1920 1080 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse auto media type", () => {
            const dsl =
                'affect auto "input.mp4" "output.mp4" { resize 1280 720 }';
            const ast = parseDsl(dsl);
    ...
    describe("Context Variables", () => {
        ...
        it("should parse variables with underscores", () => {
            const dsl =
                "affect auto $input_file $output_file { resize 1280 720 }";
            const ast = parseDsl(dsl);
    ...
    describe("Resize Command", () => {
        ...
        it("should parse resize with auto height", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { resize 1280 auto }';
            const ast = parseDsl(dsl);
    ...
    describe("Encode Command", () => {
        it("should parse video encode with codec and bitrate", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { encode h264 2000 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse encode with string param", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { encode h264 "high" }';
            const ast = parseDsl(dsl);
        ...
        it("should parse multiple encode commands", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { encode h264 2000 encode aac 128 }';
            const ast = parseDsl(dsl);
    ...
    describe("Filter Command", () => {
        it("should parse filter with name only", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { filter grayscale }';
            const ast = parseDsl(dsl);
        ...
        it("should parse filter with name and numeric value", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { filter blur 5 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse filter with name and string value", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { filter preset "slow" }';
            const ast = parseDsl(dsl);
        ...
        it("should parse multiple filters", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { filter blur 5 filter brightness 1.2 }';
            const ast = parseDsl(dsl);
    ...
    describe("Crop Command", () => {
        it("should parse crop with x, y, width, height", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { crop 0 0 640 480 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse crop with center and auto", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { crop center auto 640 480 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse crop with region string", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { crop "center" 640 480 }';
            const ast = parseDsl(dsl);
    ...
    describe("Rotate Command", () => {
        ...
        it("should parse rotate with angle and flip", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { rotate 180 "horizontal" }';
            const ast = parseDsl(dsl);
...
affect video "input.mp4" "output.mp4" {
  ...
  filter brightness 1.1
            ...
            const operations = ast.commands.filter(
                (cmd) => cmd.type !== "Input" && cmd.type !== "Save"
            );
    ...
    describe("Output Path Handling", () => {
        ...
        it("should parse save command inside block", () => {
            const dsl =
                'affect video "input.mp4" { resize 1280 720 save "output.mp4" }';
            const ast = parseDsl(dsl);
    ...
    describe("String Literals", () => {
        ...
        it("should parse strings with spaces", () => {
            const dsl =
                'affect video "my input file.mp4" "my output file.mp4" { resize 1280 720 }';
            const ast = parseDsl(dsl);
        ...
        it("should parse strings with escape sequences", () => {
            const dsl =
                'affect video "input\\"file.mp4" "output\\nfile.mp4" { resize 1280 720 }';
            const ast = parseDsl(dsl);
...
\tencode h264 2000
    ...
    describe("File Parsing - Usage Examples", () => {
        it("should parse DSL from assets/create-video.affect file (comprehensive usage example)", () => {
            ...
            if (ast.type === "AffectBlock") {
                ...
                const videoBlock = ast.commands.find(
                    (cmd) => cmd.type === "VideoBlock"
                );
                expect(videoBlock).toBeDefined();
                if (videoBlock && videoBlock.commands) {
                    const videoBlockTypes = videoBlock.commands.map(
                        (cmd) => cmd.type
                    );
                ...
                const audioBlock = ast.commands.find(
                    (cmd) => cmd.type === "AudioBlock"
                );
                expect(audioBlock).toBeDefined();
                if (audioBlock && audioBlock.commands) {
                    const audioBlockTypes = audioBlock.commands.map(
                        (cmd) => cmd.type
                    );
                ...
                const filterBlock = ast.commands.find(
                    (cmd) => cmd.type === "FilterBlock"
                );
                expect(filterBlock).toBeDefined();
                if (filterBlock && filterBlock.commands) {
                    const filterTypes = filterBlock.commands.map(
                        (cmd) => cmd.type
                    );
  ...
  audio codec "aac"
            ...
            const ast = parseDsl(dsl);
            expect(ast.commands.some((cmd) => cmd.type === "VideoBlock")).toBe(
                true
            );
            expect(ast.commands.some((cmd) => cmd.type === "AudioCodec")).toBe(
                true
            );
        });
    });
    ...
    describe("Compiler Tests", () => {
        it("should compile basic DSL to JavaScript", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { resize 1280 720 }';
            const code = compileDsl(dsl);
        ...
        it("should compile DSL with context variables", () => {
            ...
            expect(code).toContain("test.mp4");
            expect(code).toContain("out.mp4");
        });
  ...
  video {
    ...
    fps 30
            ...
            const code = compileDsl(dsl);
            expect(code).toContain("videoCodec");
            expect(code).toContain("fps");
        });

        it("should add -n option to prevent overwrite", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { resize 1280 720 }';
            const code = compileDsl(dsl);
  ...
  filter grayscale
            ...
            const code = compileDsl(dsl);
            expect(code).not.toContain("videoCodec");
            expect(code).not.toContain("audioCodec");
  ...
  audio {
    codec "aac"
            ...
            expect(code).toContain("video commands are ignored");
            expect(code).toContain("audio commands are ignored");
        });
  ...
  audio {
    codec "aac"
            ...
            const code = compileDsl(dsl);
            expect(code).not.toContain("videoCodec");
            expect(code).toContain("audioCodec");
        });
  ...
  video {
    codec "libx264"
            ...
            const code = compileDsl(dsl, { warnings: false });
  ...
  audio {
    ...
    describe("Video Commands", () => {
        it("should parse video codec command", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { video codec "libx264" }';
            const ast = parseDsl(dsl);
            const videoCodec = ast.commands.find(
                (cmd) => cmd.type === "VideoCodec"
            );
        ...
        it("should parse video bitrate command", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { video bitrate "1M" }';
            const ast = parseDsl(dsl);
            const videoBitrate = ast.commands.find(
                (cmd) => cmd.type === "VideoBitrate"
            );
        ...
        it("should parse video fps command", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { video fps 30 }';
            const ast = parseDsl(dsl);
            const videoFPS = ast.commands.find(
                (cmd) => cmd.type === "VideoFPS"
            );
    ...
    describe("Audio Commands", () => {
        it("should parse audio codec command", () => {
            const dsl =
                'affect audio "input.wav" "output.mp3" { audio codec "aac" }';
            const ast = parseDsl(dsl);
            const audioCodec = ast.commands.find(
                (cmd) => cmd.type === "AudioCodec"
            );
        ...
        it("should parse audio bitrate command", () => {
            const dsl =
                'affect audio "input.wav" "output.mp3" { audio bitrate 128 }';
            const ast = parseDsl(dsl);
            const audioBitrate = ast.commands.find(
                (cmd) => cmd.type === "AudioBitrate"
            );
        ...
        it("should parse audio channels command", () => {
            const dsl =
                'affect audio "input.wav" "output.mp3" { audio channels 2 }';
            const ast = parseDsl(dsl);
            const audioChannels = ast.commands.find(
                (cmd) => cmd.type === "AudioChannels"
            );
        ...
        it("should parse audio frequency command", () => {
            const dsl =
                'affect audio "input.wav" "output.mp3" { audio frequency 44100 }';
            const ast = parseDsl(dsl);
            const audioFrequency = ast.commands.find(
                (cmd) => cmd.type === "AudioFrequency"
            );
    ...
    describe("Options Commands", () => {
        ...
        it("should parse format command", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { format "mp4" }';
            const ast = parseDsl(dsl);
    ...
    describe("Event Handlers", () => {
        it("should parse on start event", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { on start "handler" }';
            const ast = parseDsl(dsl);
        ...
        it("should parse on end event", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { on end "handler" }';
            const ast = parseDsl(dsl);
        ...
        it("should parse on error event", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { on error "handler" }';
            const ast = parseDsl(dsl);
        ...
        it("should parse on progress event", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { on progress "handler" }';
            const ast = parseDsl(dsl);
            const onProgress = ast.commands.find(
                (cmd) => cmd.type === "OnProgress"
            );
    ...
    describe("Conditional Logic (if/else)", () => {
        it("should parse simple if statement", () => {
            const dsl =
                'affect video "input.mp4" "output.mp4" { if width > 1920 { resize 1920 auto } }';
            const ast = parseDsl(dsl);

            const ifBlock = ast.commands.find(
                (cmd) => cmd.type === "IfBlock"
            );
  ...
  if width > 1920 {
    ...
    encode h264 2000
        ...
        it("should compile if statement correctly", () => {
            ...
            expect(code).toContain("metadata.width");
  ...
  if width > 1920 and height > 1080 {
    resize 1920 1080
            ...
            expect(code).toContain("metadata.width");
            expect(code).toContain("metadata.height");
        });
    });
});
