...
describe("Assets Files Testing", () => {
  ...
  const fs = require("fs");
  const assetFiles = fs.readdirSync(assetsDir).filter((f: string) => f.endsWith(".affect"));
      ...
      it(`should parse ${file} without errors`, () => {
        ...
        expect(() => {
          ...
          if (ast.type === "AffectBlock") {
            ...
            expect(["auto", "video", "audio", "image"]).toContain(ast.mediaType);
            expect(Array.isArray(ast.commands)).toBe(true);
          } else if (ast.type === "Program" || ast.type === "ConvertBlock") {
            expect(Array.isArray(ast.commands)).toBe(true);
          }
        }).not.toThrow();
      });
  ...
  describe("Specific Asset Files Validation", () => {
    ...
    it("should compile assets with context variables correctly", () => {
      ...
      // 验证上下文变量被正确替换
      expect(compiled.input).toBe("test-input.mp4");
      expect(compiled.output).toBe("test-output.mp4");
    });
  ...
  // 测试编译后的操作质量
  describe("Compiled Operations Quality", () => {
    it("should generate valid operations for all assets", () => {
      assetFiles.forEach((file: string) => {
        const filePath = join(assetsDir, file);
        const compiled = compileDslFile(filePath, {
          context: { input: "test.mp4", output: "out.mp4" },
    ...
    it("should include output path for files with output", () => {
      // 测试有输出路径的文件
      const filesWithOutput = ["video-basic.affect", "audio-basic.affect", "image-basic.affect"];
      ...
      filesWithOutput.forEach((file: string) => {
        ...
        expect(compiled.operations).toBeDefined();
        // Should have save operation or output in compiled result
        const hasSave = compiled.operations.some((op) => op.type === "save");
        expect(hasSave || compiled.output).toBeTruthy();
      });
    });
  });
});
