...
program.name("affect").description("AffectJS CLI - Easy way to run affect DSL").version(version);

// Setup subcommand - delegates to fluent-setup
program
  .command("setup")
  .description("Setup FFmpeg 6.1.* environment (calls fluent-setup)")
  .option("-i, --install", "Automatically install/upgrade ffmpeg if needed", false)
  .option("-s, --silent", "Suppress non-error output", false)
  .option("--check-only", "Only check environment, do not install", false)
  .option("--json", "Output results as JSON", false)
  .action(
    async (opts: { install?: boolean; silent?: boolean; checkOnly?: boolean; json?: boolean }) => {
      ...
      try {
        ...
        if (opts.json) {
          console.log(
            JSON.stringify({
              ...
              config: {
                ...
                ffmpegVersion: config.ffmpegVersion ? config.ffmpegVersion.full : null,
          ...
          if (config.ffmpeg && config.available) {
            ...
            if (shell === "cmd") {
              const result = setWindowsEnvVars(config.ffmpeg, config.ffprobe || "");
              if (result.success) {
              ...
              const result = writeShellConfig(shell, config.ffmpeg, config.ffprobe || "");
              if (result.success) {
                console.log(`  ✓ ${result.message}`);
        ...
        if (config.ffmpegVersion && !isVersionSupported(config.ffmpegVersion)) {
  ...
  .action(
    async (
      ...
      try {
        ...
        if (ext !== ".affect") {
          console.error(`Error: Invalid file type. Expected .affect file, got: ${ext}`);
          process.exit(1);
        }
        ...
        const { execute, affect, affectBatch } = await import("@affectjs/runtime-node");
