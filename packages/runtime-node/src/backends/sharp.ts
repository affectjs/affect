...
export const sharpBackend: Backend = {
  ...
  applyOperation(command: unknown, operation: Operation, mediaType: MediaType): unknown {
    switch (operation.type) {
      case "resize": {
        ...
        const h =
          operation.height === "auto" || !operation.height ? null : Number(operation.height);
        return command.resize(w, h);
      }
      case "encode":
        return command.toFormat(
          operation.codec,
          operation.param ? { quality: Number(operation.param) } : {}
        );
      case "filter":
        switch (operation.name) {
          ...
          default:
            if (operation.value !== undefined && operation.value !== null) {
              return (command as unknown)[operation.name]?.(Number(operation.value)) || command;
            } else {
              return (command as unknown)[operation.name]?.() || command;
            }
        }
      case "crop": {
  ...
  async getMetadata(input: string): Promise<unknown> {
      ...
      for (const op of context.operations) {
        switch (op.type) {
          case "resize": {
            ...
            const h = op.height === "auto" || !op.height ? null : Number(op.height);
            pipeline = pipeline.resize(w, h);
            break;
          }
          case "encode":
            pipeline = pipeline.toFormat(op.codec, op.param ? { quality: Number(op.param) } : {});
            break;
          case "filter":
            switch (op.name) {
              case "grayscale":
                ...
                break;
              case "blur":
                pipeline = pipeline.blur(op.value ? Number(op.value) : 1);
                break;
              case "brightness":
              ...
              case "contrast":
                pipeline = pipeline.linear(
                  ...
                  -(128 * (op.value ? Number(op.value) : 1)) + 128
                );
                break;
              default:
                // Try to call as sharp method
                if (op.value !== undefined && op.value !== null) {
                  (pipeline as unknown)[op.name]?.(Number(op.value));
                } else {
                  (pipeline as unknown)[op.name]?.();
                }
            }
            break;
          case "crop": {
